...
class DecoupledMVRowSelfAttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        query = attn.to_q(hidden_states)#(12,4096,320)->(12,4096,320)

        # NEW: for decoupled multi-view attention
        if use_mv:
            query_mv = self.to_q_mv(hidden_states)#(12,4096,320)->(12,4096,320)

        # NEW: for decoupled reference cross attention
        if use_ref:
            query_ref = self.to_q_ref(hidden_states)#(12,4096,320)->(12,4096,320)
        ...
        key = attn.to_k(encoder_hidden_states)#(12,4096,320)->(12,4096,320)
        value = attn.to_v(encoder_hidden_states)#(12,4096,320)->(12,4096,320)

        inner_dim = key.shape[-1]#320
        head_dim = inner_dim // attn.heads#64

        query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)#(12,4096,320)->(12,5，4096,64)

        key = key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)#(12,4096,320)->(12,5，4096,64)
        value = value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)#(12,4096,320)->(12,5，4096,64)
        ...
        )#(12,5，4096,64)->(12,4096,320)
        hidden_states = hidden_states.to(query.dtype)

        ####### Decoupled multi-view self-attention ########
        if use_mv:
            key_mv = self.to_k_mv(encoder_hidden_states)#(12,4096,320)->(12,4096,320)
            value_mv = self.to_v_mv(encoder_hidden_states)#(12,4096,320)->(12,4096,320)

            query_mv = query_mv.view(batch_size, -1, attn.heads, head_dim)#(12,4096,320)->(12,4096,5,64)
            key_mv = key_mv.view(batch_size, -1, attn.heads, head_dim)#(12,4096,320)->(12,4096,5,64)
            value_mv = value_mv.view(batch_size, -1, attn.heads, head_dim)#(12,4096,320)->(12,4096,5,64)
            ...
            query_mv = rearrange(
                ...
                ih=height,
                iw=width,
            ).transpose(1, 2)#(12,4096,5,64)->(768,5,64,64)
            key_mv = rearrange(
                key_mv,
