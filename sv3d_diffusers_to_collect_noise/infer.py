...
def main():
    ...
    parser.add_argument("--output_dir", default="standard_sv3d_gso100_elevation10_outputs", type=str, help="Output filepath")
    parser.add_argument("--input_paths", default="gt_gso100_elevation0_for_sv3d", type=str, help="Image filepath")
        ...
        if os.path.exists(folder_dir) and os.path.isdir(folder_dir):
            ...
            with torch.no_grad():
                with torch.autocast("cuda", dtype=torch.float16 if args.half_precision else torch.float32,
                    ...
                    org_latents, gt_latents,image_latenets_without_noise = pipeline(
                        input_image,
                        height=sv3d_res,
                        width=sv3d_res,
            ...
            org_latents_path = os.path.join(image_folder, 'org_latents.pt')
            gt_latents_path = os.path.join(image_folder, 'gt_latents.pt')
            image_latenets_without_noise_path = os.path.join(image_folder, 'image_latenets_without_noise.pt')
            torch.save(org_latents, org_latents_path)
            torch.save(gt_latents, gt_latents_path)
            torch.save(image_latenets_without_noise, image_latenets_without_noise_path)
